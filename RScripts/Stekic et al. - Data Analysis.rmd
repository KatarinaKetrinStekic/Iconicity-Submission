---
title: "Stekic et al. - Words of a Feather Flock Toghether - Data Analysis"
output:   
  html_document:
    keep_md: true
    theme: journal
...
CleanData <- read.csv("D:/WIN/Documents/GitHub/Stekic-et-al/Data/CleanDataFinal.csv")
...
For the final part of the study we bring everything from the above together only graphically to show for an omnibus difference between item and category conditions in regard to the no label and arbitrary conditions.
...
But we know from our omnibus analysis that level of Iconicity makes no difference (and of course we saw that in all other analyses), so lets plot Label Type vs Systematicity instead. 


```{r Omnibus - Label Type x Systematicity Plot}
